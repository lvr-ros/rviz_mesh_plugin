...
namespace rviz_mesh_plugin
{
  ...
  TriangleMeshVisual::TriangleMeshVisual(rviz::DisplayContext* context,
      size_t displayID,
      size_t meshID)
    ...
    m_normalsScalingFactor(-1)
  {
    ...
    if (sceneManager->hasSceneNode("TriangleMeshScene")){
      m_sceneNode = (Ogre::SceneNode*)(rootNode->getChild("TriangleMeshScene"));
    }else{
      m_sceneNode = rootNode->createChildSceneNode("TriangleMeshScene");
    } 

    // create manual object and attach it to the scene node
    std::stringstream sstm;
    sstm << m_prefix << "_TriangleMesh_" << m_postfix;
  ...
  void TriangleMeshVisual::reset()
  {
    ...
    {
      sstm << m_prefix << "_TriangleMesh_" << m_postfix << "Texture_" << i;
    ...
    sstm << m_prefix << "_TriangleMesh_" << m_postfix << "Material_" << 0;
    Ogre::MaterialManager::getSingleton().unload(sstm.str());
    Ogre::MaterialManager::getSingleton().remove(sstm.str());
    sstm.str("");
    sstm.clear();

    sstm << m_prefix << "_TriangleMesh_" << m_postfix << "Material_" << 1;
    Ogre::MaterialManager::getSingleton().unload(sstm.str());
    Ogre::MaterialManager::getSingleton().remove(sstm.str());
    sstm.str("");
    sstm.clear();

    sstm << m_prefix << "_TriangleMesh_" << m_postfix << "NormalMaterial";
    Ogre::MaterialManager::getSingleton().unload(sstm.str());
    Ogre::MaterialManager::getSingleton().remove(sstm.str());
    sstm.str("");
    sstm.clear();
  ...
  void TriangleMeshVisual::showWireframe(
    ...
    float wireframeAlpha
        ...
        pass->setDiffuse(0,0,0,wireframeAlpha);
  ...
  void TriangleMeshVisual::showFaces(
    ...
    pass->setDiffuse(0,0,0,facesAlpha);
    if(useVertexColors){
  ...
  void TriangleMeshVisual::showNormals(
    ...
    pass->setDiffuse(0,0,0,normalsAlpha);
    if (normalsAlpha < 1.0){
      pass->setSceneBlending(Ogre::SBT_TRANSPARENT_ALPHA);
      pass->setDepthWriteEnabled(false);
    }
        ...
        float normalsScalingFactor)
  {
    ...
    if(!m_normalMaterial.isNull()){
      if (showNormals){
        ...
        if (normalsScalingFactor != m_normalsScalingFactor){
...
void TriangleMeshVisual::setMessage(const mesh_msgs::TriangleMeshStamped::ConstPtr& mesh_msg)
{
  ...
  if(mesh.vertex_colors.size() == mesh.vertices.size()){
    ROS_INFO("Received %d vertex colors.", mesh.vertex_colors.size());
  ...
  if(mesh.triangle_colors.size() == mesh.triangles.size()){
    ROS_INFO("Received %d triangle colors.", mesh.triangle_colors.size());
  ...
  if(mesh.vertex_texture_coords.size() == mesh.vertices.size()){
    ROS_INFO("Received %d texture coords.", mesh.vertex_texture_coords.size());
  ...
  if(mesh.vertex_normals.size() == mesh.vertices.size()){
    ROS_INFO("Received %d vertex normals.", mesh.vertex_normals.size());
